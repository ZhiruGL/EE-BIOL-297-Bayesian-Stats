...
#The effect of predict is 10.86 suggesting that when when predict increase by 1, the number of penguins counted will increase by 10.86. However, the p-value for the predict is 0.813 which is greater than 0.05, thus it is insignificant at the significance level of 0.05. This is what we would expect as predict is totally random, thus do not actually carry and ecological/biological information, thus the cannot actually predict the penguin counts
```
...
penguin.mod1 <- function() {
  ...
  for(i in 1:n.obs) {
<<<<<<< HEAD
    mu[i] <- b0 + b1*predictor[i]
=======
    mu[i]=b0+b1*predictor[i]
    
>>>>>>> c9cdb8f53f7c06292154ff471f929bc0d2b7fce1
...
<<<<<<< HEAD
#We can learn from this plot that the data did not actually improve our understanding of the distribution of b0 and b1 very well. The overlap between priors and posteriors for b0 and b1 is 45.6% and 88.1%, respectively, hence the model actuall fits poorly. Given that we are using vague priors and according to the PPO plot, the priors appear to not to be flat across the estimated region of both posteriors; thus, I would conclude that the priors do not actually look vague.
=======
#We can learn from this plot that the data fail to improve our understanding of the distribution of b0 and b1 significantly. The overlap between priors and posteriors for b0 and b1 is 45.6% and 88.1%, respectively. Given that we are using vague priors and according to the PPO plot, the priors appear to not be flat across the estimated region of both posteriors; thus, I would conclude that the priors does not look vague.
>>>>>>> c9cdb8f53f7c06292154ff471f929bc0d2b7fce1
```
...
>>>>>>> c9cdb8f53f7c06292154ff471f929bc0d2b7fce1
    penguin_count[i] ~ dnorm(mu[i], tau)
    penguin.predict[i] ~ dnorm(mu[i], tau)
  }  
}
penguin.fit_pred<- jags(data=jags.data, 
                    parameters.to.save=c("b0","b1","tau","penguin.predict"),
...
hist(sim_mean,breaks=20,main="Bayesian p-value",xlab="Simulated Means",ylab="Frequency",xlim=c(-600,600))
...
<<<<<<< HEAD
#According to the result, the p_value is 0, which is very small and far away from 0.5, thus suggest that our model is poorly fit to the data, and the unlikely to reproduce the observed data.
=======
#According to the result, the p_value is 0, which is very far from 0.5, thus suggest that our model is a poor fit to the data.
>>>>>>> c9cdb8f53f7c06292154ff471f929bc0d2b7fce1
```
...
<<<<<<< HEAD
                    parameters.to.save=c("b0","b1","penguin.predict"),
=======
                    parameters.to.save=c("b0","b1","tau","penguin.predict"),
>>>>>>> c9cdb8f53f7c06292154ff471f929bc0d2b7fce1
...
<<<<<<< HEAD
#We can learn from this plot that the data actually improve our understanding of the distribution of b0 and b1. The overlap between priors and posteriors for b0 and b1 are both 10.5% and 10.9% respectively. Given that we are using vague priors and according to the PPO plot, the priors appear to be flat across the estimated region of both posteriors (much faltter compare to the previous model); thus, I would conclude that the priors actually look vague. Furthermore, the prior is more vague compare to the previous used ones as the overlapping ratio decreased significantly, thus the observed data is improving our understanding about the distribution of the paramemter
=======
#We can learn from this plot that the data actually improve our understanding of the distribution of b0 and b1. The overlap between priors and posteriors for b0 and b1 are both 10.5% and 10.9% respectively which is lower the the previous one. Given that we are using vague priors and according to the PPO plot, the priors appear to be flat across the estimated region of both posteriors; thus, I would conclude that the priors actually look vague. Furthermore, the prior is more vague compare to the previous used ones as the overlapping ratio decreased significantly.
>>>>>>> c9cdb8f53f7c06292154ff471f929bc0d2b7fce1

#According to the result, the p_value is 0.471, which is very close to 0.5, thus suggest that our model is a good fit to the data.
```
...
=======
#The plot looks pretty bad, although the mean values are roughly equally distributed above/below the 1:1 line, which accounted for the observed Bayesian p-value of roughly 0.5 in both models. They fail to follow the 1:1 line; instead, there is a constant overestimation for the count in early colonies, whereas there is a constant underestimation of the count in later colonies. Furthermore, the confidence interval is very large compared to the counts themselves, which provides a high coverage. However such large confidence interval is also suggesting that the model fails to provide a precise and accurate prediction of the penguin counts in each colony, and the count is still largely random. Thus suggesting that the proposed predictor (the random normally distributed one), is a very bad predictor for our model in this case.

# Interestingly, the predicted value of the penguin counts oscillates around 500 and seems to have a difficult time getting away from it. This is unsurprising, as in the original dataset the population mean in the negative binomial distribution is set to 500. Thus, the model faithfully reflected that. However, the fact that the model cannot seem to deviate from that suggests that either the predictors are not powerful enough to actually simulate the variation within the population, or there are hidden variables that remain to be discovered. In this case, it is likely that both cases are involved. Since we are using a random predictor drawn from a normal distribution, it certainly does not contain any ecological/biological significance, thus has poor explanatory power. Furthermore, since we are only using one predictor without considering all the other possibilities of predictors, we are ignoring the ones that could actually explain the pattern, such as important ecological processes (however, our data are randomly generated, so this is only for the purpose of illustration). To conclude, the model is actually a very poor fit, representing the sampled value as well as the population.
...
ggplot(df_2,aes(x=obs,y=sim_mean))+
  ...
  geom_abline(intercept=0,slope=1,linetype="dashed",color="red")+
  coord_cartesian(xlim=c(0,1200),ylim=c(0,1200))+
  labs(x="Observed penguin count",y="Simulated penguin count",title ="n.pred=2")

#It helped as the Bayesian p-value is now 0.489, which is closer to 0.5 compared to the previous 0.472 when only 1 predictor was used. According to the dot plot, the predictions are also closer to the 1:1 line which also suggested a better fit.
```
...
hist(sim_mean_mods_15,breaks=20,main="n.pred=15",xlab="Simulated Means",ylab="Frequency")
...
```{r, echo = TRUE}
<<<<<<< HEAD
#All the dots are very close on the 1:1 line, suggesting that the model can generate data patterns that are very similar to the observed value. This could be a good thing, as the model is really good at predicting the dataset itself; however, this could be a bad thing as well, suggesting a pattern of overfitting. Another bad thing observed in the model is that the confidence interval for each prediction is still large, suggesting that there is still huge uncertainty within the posterior distribution. Thus, although the model did a good job in predicting the dataset, this is solely due to the addition of the predictors, which does not necessarily increase predictive accuracy and precision but only creates an illusion of fit with a huge remained uncertainty. Thus although the model can predict the sampled datasets really well using the mean of the distribution, it does not really predict well and tell us much about the real populaiton distribution.

#The predictions are not exact because there still is uncertainty within the posterior distribution, as demonstrated in the histogram for n.preds=30, where not all the posterior simulated values are concentrated at the value of the observed mean; instead, there are still values scattered around due to the innate uncertainty in the Bayesian inference process. 
=======
#All the dots are very close on the 1:1 line, suggesting that the model can generate data patterns that are very similar to the observed value. This could be a good thing, as the model is really good at predicting the data set, which is not surprising as there are more than 30 paramemters for 30 datapoints thus the model is capable of fine tuning each predicted data points to the observed data sampled. Furthermore, for every single point, the confidence interval is overlapping with the 1:1 line suggesting a 100% coverage. This could be a good thing as the “correct" predicted value is included in the confidence interval. However, in this case, this coverage could be attributed to the large confidence interval which is not a good thing as described below. In the model, the confidence interval for each prediction is still large, suggesting that there is still huge uncertainty within the posterior distribution. Together with the almost perfect fit of the predicted means to the 1:1 line, this suggests a pattern of overfitting, which is a bad thing.The pattern suggested that although the model did a good job in predicting the mean, this is solely due to the addition of the predictors, which does not necessarily increase predictive power but only creates an illusion of fit and actually increases uncertainty. 

#The predictions are not exact because there still is uncertainty within the posterior distribution. As demonstrated in the histogram for n.preds=30@ not all the posterior simulated values are concentrated at the value of the observed mean; instead, there are still values scattered around due to the innate uncertainty in the Bayesian inference process. 
>>>>>>> c9cdb8f53f7c06292154ff471f929bc0d2b7fce1
```
...
```{r, echo = TRUE}
<<<<<<< HEAD
#This exercise illustrates the fact that covariates should not be added in attempts to increase model fitting, because it creates an illusion of better fit without actually increasing the precision and accuracy of the model, hence overfitting. This is hard to see with frequentist analysis, as they treat all the parameters and inferences as fixed values; thus, the result of adding variables will only be reflected as a better fitted p-value if the analysis is not thorough (thus we see other tests that include degrees of freedom as a parameter in the test statistics). However, in Bayesian stats, the results of adding new variables are more obvious, as it only improves the accuracy of the mean but not precision that concerned with the confidence interval of the posterior distribution, thus suggesting that the "better fitted model" was only an illusion created due to higher complexity of the model. To conclude, I think a better approach in model building, as suggested by the exercise itself, is a rule of thumb that one needs at least 10 data points for every covariate. However, since this could be jeopardized by the issue of pseudoreplication, I would suggest that one only tries to add variables if it makes biological sense or is relevant to the study itself.
=======
#This exercise illustrates the fact that covariates should not be added in attempts to increase model fitting, because it creates an illusion of a better fit without actually increasing the precision and accuracy of the model, hence overfitting. This is hard to see with frequentist analysis, as they treat all the parameters and inferences as fixed values; thus, the result of adding variables will only be reflected as a better-fitted p-value if the analysis is not thorough (thus we see other tests that include degrees of freedom as a parameter in the test statistics). However, in Bayesian stats, the results of adding new variables are more obvious, as it only improves the accuracy of the mean but not the posterior distribution, thus suggesting that the "better-fitted model" was only an illusion created due to higher complexity of the model. To conclude, I think a better approach in model building, as suggested by the exercise itself, is a rule of thumb that one needs at least 10 data points for every covariate. However, since this could be jeopardized by the issue of pseudoreplication. According to the conclusion drawn from A7, another perspective to take when considering the question is that, instead of increasing the number of "rubbish"/meaningless predictors (like the random ones we are using here), we could try to include predictors that actually are supposed to have stronger explanatory power, which could help the model to do better in predicting the dataset and make inference about the population parameters. Hence, I would suggest that one only tries to add variables if it makes biological sense or is relevant to the study itself. All in all, this comes to the classic slang in modelling: Garbage in, garbage out.
>>>>>>> c9cdb8f53f7c06292154ff471f929bc0d2b7fce1
```
