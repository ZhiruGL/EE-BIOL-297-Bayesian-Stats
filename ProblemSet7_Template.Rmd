...
qs_mod<-function(){
  ...
  for (j in 1:site_num){
    N[j] ~ dpois(lam[j])
    log(lam[j])<-b0+b_elev*elev[j]+b_forest*forest[j]+b_int*elev[j]*forest[j]
...
qs.fit<-jags(data=jags_data_qs,parameters.to.save=c("b0", "b_elev","b_forest","b_int","alpha_0","alpha_wind","alpha_forest"),model.file=qs_mod,
...
```{r}
MCMCsummary(qs.fit)
#The mean of b0 is 0.635, meaning that the average count for all sites, without the consideration of all the other effect is e^0.635

#Elevation has a highly-certain and strong negative impact on quail counts. The mean of b_elev is -2.064. Thus, when elevation increase by 1, the mean count of the site will change by a factor of e^-2.064

#Both canopy cover and the interaction between canopy cover and elevation have a positive impact on the count fo quails, and we are very much confident about this effect. On average, when canopy cover and the interaction between canopy cover and elevation increase by 1 unit, the expected abundance of quails at that site is increase by a factor of e^2.14 and e^1.29, respectively

#alpha_0's effect on observation probability is strongly negative with high certainty. The mean of alpha_0 is -0.912, thus the average odd (not probability) of successful observation without considering other effect is e^-0.912.

#alpha_wind has a very strong negative and highly certain effect on observation success. The mean of alpha_wind is -2.97, suggesting that the average odd of successful observation would change by a factor of e^-2.97 when wind increase by 1 unit. Thus we are highly confident that wind negatively impacts our observation success for quails

#alpha_forest has a 95% confidence interval overlap with 0, thus we are uncertain whether it has any actual impact on the observation success or not. Moreover the mean value is small: 0.0143. Thus it is safe to say that canopy cover does not actually contribute much to the detection process of quails
...
#The p values are 0.474 and 0.653
