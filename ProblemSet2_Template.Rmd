...
print(var_total)
print(expected_value)
#For Poisson distribution variance is equal to expected value which is 0.225. However the sample variance is 0.724, which is greater than 0.225, thus the empirical frequency distribution is over-dispersed whenn compared to the expected frequnecy distribution.
...
# To clarify, an optimal plot for comparison here would be a barplot, however it seems that since the value of the frequency of 1-count and the frequency of 3-6 counts differ too much, bar plots are not visually informative, thus here scatter plot was used. AS the plot demonstrated, the observed frequency (blue triangle) are lower than the expected frequency (red dots) when the number of groundhogs counted are 2; and the observed frequency (blue triangle) are higher then the expected frequency (red dots) when the number of groundhogs counted are high(4-6).Thus it could be concluded that the empirical frequency distribution is over-dispersed when compared to the expected frequency distribution
```
...
lik_all=c()
for (i in 1:6){
  lik_indiv=c(rep(p_mass[i],marmots$Frequency[i]))
...
#This maximum likelihood value is the same as the sample mean.
```
...
# Your answer here
# Gamma distribution should be used to model response variables that are: non-negative, right-skewed/have increased variance with increased value
# First of all, the response parameter of the gamma distribution will be non-negative, which is well suited for the purpose of measuring growth.
# secondly, as demonstrated in the histogram, the distribution is right-skewed, thus gamma distribution would be appropriate as the variance of the response variable would increase together with increased value.
